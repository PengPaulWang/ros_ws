...
# Ollama client
MODEL = "llama3"
client = OllamaFunctions(
    # cat /etc/resolv.conf - way to find gateway ip somewhat
    base_url="http://172.19.0.1:11434",
    model=MODEL,
    keep_alive="10m",  # keep the model alive for 10 minutes, -1 for infinite
    format="json",  # https://python.langchain.com/v0.2/docs/integrations/chat/ollama_functions/
)
...
def run_conversation(user_prompt):
    # maybe add that to the other file aswell?
    global client
    ...
    function_call = {"name": "send_cmd_vel"}

    client = client.bind_tools(tools=tools, function_call=function_call)
